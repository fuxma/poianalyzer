...
(function ()
{
  ...
  injectScript.innerHTML = "(" + function ()
  {
      ...
      qx.Class.define('ccta_pa',
        {
          ...
          construct: function ()
          {
            try {
              ...
              for (var i = 1; i < 41; i++)
                tiersData.push([i, '+' + getMultiplier(i) + '%']);
              ...
              for (var i = 0; i < 7; i++) {
                ...
                for (var n = 0; n < columns.length; n++) {
                  if (n === 0)
                    renderer = new qx.ui.table.cellrenderer.Html();
              ...
              for (var n = 0; n < 5; n++) {
                if (n == 1)
                  rankingTable.getTableColumnModel().setDataCellRenderer(n, new qx.ui.table.cellrenderer.Html());
                else
                  rankingTable.getTableColumnModel().setDataCellRenderer(n, new qx.ui.table.cellrenderer.Default().set(
                    {
                      useAutoAlign: false
                    }
                  ));
              ...
              label.setValue(String.fromCharCode(77) + [65, 68, 69, 32, 66, 89, 32, 90, 68, 79, 79, 77
                ].map(codeToString).join().replace(/,/g, ''));
              ...
              for (var n = 0; n < 4; n++) {
                if (n === 0)
                  poisTable.getTableColumnModel().setDataCellRenderer(n, new qx.ui.table.cellrenderer.Html());
                else if (n === 3)
                  poisTable.getTableColumnModel().setDataCellRenderer(n, new qx.ui.table.cellrenderer.Boolean());
                else
                  poisTable.getTableColumnModel().setDataCellRenderer(n, new qx.ui.table.cellrenderer.Default().set(
                    {
                      useAutoAlign: false
                    }
                  ));
              ...
              poisTable.getSelectionModel().addListener('changeSelection', function (e)
              {
                ...
                for (var i = 0; i < data.length; i++) {
                  ...
                  if (!isSelected)
                    score += getScore(parseInt(level, 10));
                }
              ...
              for (var i = 12; i <= maxPoiLevel; i++)
                selectLevel.add(new qx.ui.form.ListItem('Level ' + i, null, i));
              ...
              selectBox.addListener('changeSelection', function (e)
              {
                if (!e.getData()[0])
                  return;
                this.__selectedSimPoi = e.getData()[0].getModel();
                this.__initSim();
              }, this);
          ...
          members: {
            ...
            __element: function (tag)
            {
              ...
              this.append = function ()
              {
                for (var i in arguments) {
                  if (arguments[i].__instanceof == 'element')
                    root.elm.appendChild(arguments[i].elm);
                  else if (arguments[i] instanceof Element)
                    root.elm.appendChild(arguments[i]);
                  else
                    console.log(arguments[i] + ' is not an element');
                }
              };
                ...
                opois.map(function (poi)
                  {
                    if (poi.t - startRank === i) {
                      var a = webfrontend.gui.util.BBCode.createCoordsLinkText(
                        (poi.x + ':' + poi.y), poi.x, poi.y);
            ...
            __updateRanks: function ()
            {
              ...
              var root = this;
              var allianceName = this.__allianceName;
              ...
              if (startRank)
                for (var n = 0; n < 7; n++)
                  getPoiRanks(poiInfo(getPoiRankType(n + startRank)).type, n + startRank, n);
            },
            __setSimLabels: function ()
            {
              ...
              if (pois[type]) {
                ...
                labels[1].setValue((pois[type].tier === undefined) ? "0" : pois[type].tier);
            ...
            __setRankingRows: function (score)
            {
              ...
              var getRankingData = function (i, type, nr)
              {
                ...
                totalBonus = (pois[type].bonusType == 1) ? format(
                  Math.round(totalBonus)) : Math.round(totalBonus * 100) / 100 + '%';
                return [nr, name, format(score), '+' + multiplier + '%', totalBonus];
              };
              var getMyRanking = function (s, i, p)
              {
                var b = getBonus(pois[p].index, s);
                var m = getMultiplier(i);
                var tb = b + (b * m / 100);
                tb = (pois[p].bonusType == 1) ?
                  format(Math.round(tb)) :
                Math.round(tb * 100) / 100 + '%';
              ...
              var getRankingRows = function (s, type)
              {
                ...
                for (var i = 0; i < isolatedRanks[type].length; i++) {
                  if (s >= (isolatedRanks[type][i].pois || 0)) {
                    ...
                    if (i === 0)
                      rows = [getMyRanking(s, i + 1, type), matched, nextMatched];
                    else
                      rows = [preMatched, getMyRanking(s, i + 1, type), matched];
                    break;
                  }
                }
                return rows;
              ...
              if (rankingRows)
                this.__rankingModel.setData(rankingRows);
            },
            ...
            __setResultsRows: function (score)
            {
              ...
              var getNextRank = function (s, t)
              {
                for (var i = 0; i < ranks[t].length; i++)
                  if (s >= (ranks[t][i].pois || 0))
                    return (ranks[t][i - 1]) ? ranks[t][i - 1].pois : s;
              };
              var getPreviousRank = function (s, t)
              {
                for (var i = 0; i < ranks[t].length; i++)
                  if (s >= (ranks[t][i].pois || 0))
                    return (ranks[t][i].pois || 0);
              };
              var getSimulatedData = function (s, p)
              {
                ...
                var tb = b + (b * m / 100);
                var sc;
                sc = function (val, org, poiType, fac)
                {
                  ...
                  if (val === undefined)
                    return null;
                  if (org === undefined)
                    return st(cs[0]) + val + et;
                  else if (org !== undefined && poiType === null)
                    return ((val - org) * fac > 0) ? st(cs[1]) +
                    val + et : ((val - org) * fac < 0) ? st(cs[2]) +
                    val + et : val;
                  else {
                    var fv = (poiType == 1) ? format(Math.round(val)) :
                    Math.round(val * 100) / 100 + '%';
                    return ((val - org) * fac > 0) ? st(cs[1]) + fv +
                    et : ((val - org) * fac < 0) ? st(cs[2]) + fv +
                    et : fv;
                  }
                };
                var rows = ['Score', 'Tier', 'Rank', 'Multiplier',
                  'Previous Rank', 'Next Rank', 'Previous Tier',
                  'Next Tier', 'Bonus', 'Total Bonus'
                ];
                var data = [f(s), sc(t, ot, null, 1), sc(r, or, null, -
                  1), '+' + m + '%', '+' + f(pr), '-' + f(nr),
                  '+' + f(pt), '-' + f(nt), sc(b, ob, pp, 1), sc(tb,
                    otb, pp, 1)
                ];
                var results = [];
                for (var i = 0; i < rows.length; i++)
                  results.push([sc(rows[i]), data[i]]);
                return results;
              };
              var resultsRows = getSimulatedData(score, type);
              if (resultsRows)
                this.__resultsModel.setData(resultsRows);
            },
            ...
            __setPoisRows: function ()
            {
              ...
              if (poisRows)
                this.__poisModel.setData(poisRows);
            },
            ...
            __initSim: function ()
            {
              ...
              this.__selectPoiLevel.setSelection([this.__selectPoiLevel
                .getSelectables()[0]
              ]);
            },
            ...
            __updateGraph: function ()
            {
              try {
                ...
                var getHeight = function (s)
                {
                  ...
                  for (var i = 0; i < tiers.length; i++)
                    if (s >= tiers[i][1] && s < tiers[i][2])
                      return Math.round((s - tiers[i][1]) / (tiers[i][2] - tiers[i][1]) * 100);
                };

                var colors = ["#8dc186", "#5b9dcb", "#8cc1c7",
                  "#d7d49c", "#dbb476", "#c47f76", "#928195"
                ];
                for (var i = 0; i < ranks.length; i++) {
                  ...
                  var f_totalBonus = (poiType == 1) ? format(totalBonus) :
                  Math.round(totalBonus * 100) / 100 + ' %';
                  nextTotalBonus = (poiType == 1) ? format(
                    nextTotalBonus) : Math.round(nextTotalBonus * 100) /
                  100 + ' %';
                  pois[name] =
                  {
                    'score': score,
    ...
    function initialize_ccta_pa()
    {
      try
      {
        console.log('poiAnalyzer initializing...');
          ...
          typeof qx.core.Init != 'undefined')
        {
            ...
            ClientLib.Data.MainData.GetInstance().get_Server().get_WorldId() !== 0)
          {
            ...
            {
              console.group('poiAnalyzer');
              console.log('Error in initialize_ccta_pa', e);
              console.groupEnd();
            }
          }
          else
            window.setTimeout(initialize_ccta_pa, 1000);
        }
        else
          window.setTimeout(initialize_ccta_pa, 1000);
      }
      ...
      {
        console.group('poiAnalyzer');
        console.log('Error in initialize_ccta_pa');
        console.groupEnd();
